...
async function addCartCapacityField() {
  ...
  try {
    ...
    const escapedColumnName = columnToAdd.name.replace(/[^a-zA-Z0-9_]/g, '');
    const [rows] = await connection.execute(
      `SHOW COLUMNS FROM dddev_recipe LIKE '${escapedColumnName}';`,
    );
    if (Array.isArray(rows) && rows.length > 0) {
      console.log(
        `Column '${columnToAdd.name}' already exists in dddev_recipe. Skipping.`,
      );
    } else {
      const alterSql = `ALTER TABLE dddev_recipe ADD COLUMN ${escapedColumnName} ${columnToAdd.definition};`;
      await connection.execute(alterSql);
      console.log(`Column '${columnToAdd.name}' added successfully.`);
    }
  } catch (error) {
    console.error('Error adding cart_capacity field:', error);
    process.exit(1);
  } finally {
    if (connection) {
